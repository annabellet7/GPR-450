...
//-----------------------------------------------------------------------------
	...
	a3ui32 const hierarchyClipCount1 = 0;
	a3ui32 const hierarchySampleCount1 = 1;
...
//-----------------------------------------------------------------------------
	...
	else if (a3fileStreamOpenWrite(fileStream, animationStream))
	{
		...
		a3hierarchySetNode(scene->sceneGraph,  5, 0, "scene_teapot1");
...
//-----------------------------------------------------------------------------
	...
	scene->obj_teapot->sceneGraphIndex = a3hierarchyGetNodeIndex(scene->sceneGraph, "scene_teapot1");
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
	...
	// initialize clip pool
	a3ui32 const hierarchyKeyframeCount1 = hierarchySampleCount - 1;
	a3ui32 const additionalSampleCount1 = 6;
	a3ui32 const additionalKeyframeCount1 = additionalSampleCount - 1;
	a3ui32 const additionalClipCount1 = 1;
	a3clipPoolCreate(scene->clipPool1,
		hierarchyClipCount1 + additionalClipCount1,
		hierarchyKeyframeCount1 + additionalKeyframeCount1,
		hierarchySampleCount1 + additionalSampleCount1);
	...
	// initialize additional clips and controllers
	{
		...
		for (j = 0; j < additionalKeyframeCount1; ++j)
			a3keyframeInit(&scene->clipPool1->keyframe[additionalKeyframeStart + j],
				&scene->clipPool1->sample[additionalSampleStart + j], &scene->clipPool1->sample[additionalSampleStart + j + 1], fps_additional);

		j = additionalClipStart;
		a3clipInit(&scene->clipPool1->clip[j], "teapot_morph1",//maybe change back teapotmorph if problems arise
			&scene->clipPool1->keyframe[additionalKeyframeStart + 0],
			&scene->clipPool1->keyframe[additionalKeyframeStart + 4]);
		a3clipCalculateDuration(scene->clipPool1, j, fps_additional);

		j = a3clipGetIndexInPool(scene->clipPool1, "teapot_morph1");
		a3clipControllerInit(scene->clipCtrl_morph1, "teapot_ctrl_morph1", scene->clipPool1, j, rate_additional, fps_additional);
		scene->morph_time1 = 0.0;
	}
...
void a3animation_loadValidate(a3_DemoState* demoState, a3_Scene_Animation* scene)
{
	...
	a3clipControllerRefresh(scene->clipCtrl_morph1, scene->clipPool1);
